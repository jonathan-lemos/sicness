...
var SicCsectTab = (function () {
    ...
    SicCsectTab.prototype.makeObj = function () {
        var s = [];
        var mkH = function (len, startData) {
            if (startData === undefined) {
                startData = { name: "", loc: 0 };
            }
            return "H" + startData.name + " " + exports.asWord(startData.loc) + exports.asWord(len);
        };
        ...
        var mkE = function (startData) {
            return "E" + (startData !== undefined ? exports.asWord(startData.loc) : "");
        };
    ...
    function SicCompiler(lines) {
        ...
        lines.forEach(function (val) {
            try {
                ...
                var instr = void 0;
                split.args.replace(/(#|@|=)\*$/, "$1" + _this.csects.current.useTab.aloc.toString(10));
                for (var _i = 0, _a = Object.keys(_this.csects.current.equTab); _i < _a.length; _i++) {
                    var key = _a[_i];
                    if (split.args.match(key) === null) {
                        continue;
                    }
                    for (var s = _this.csects.current.equTab[key]; s !== undefined; s = _this.csects.current.equTab[s]) {
                        split.args = split.args.replace(key, _this.csects.current.equTab[key]);
                    }
                    break;
                }
                if (split.tag !== undefined) {
                    _this.csects.current.tagTab[split.tag] = _this.csects.current.useTab.aloc;
                }
                if (_this.csects.isDirective(split.op)) {
                    _this.csects.directives[split.op](val, split);
                    return;
                }
                if (SicFormat1.isFormat1(split.op)) {
                    ...
                    instr = new SicFormat3(split, _this.csects.current.litTab, _this.csects.current.base);
                }
                else if (SicFormat4.isFormat4(split.op)) {
                    instr = new SicFormat4(split, _this.csects.current.litTab);
                }
                else if (SicFormatLegacy.isFormatLegacy(split.op)) {
                    instr = new SicFormatLegacy(split, _this.csects.current.litTab);
                }
                ...
                _this.csects.addLst(new SicLstEntry(val, { aloc: _this.csects.current.useTab.aloc, rloc: _this.csects.current.useTab.rloc, inst: instr }));
                _this.csects.current.useTab.inc(instr.length());
            }
            catch (e) {
                _this.errflag = true;
                _this.csects.current.lst.push(new SicLstEntry(val, e.message));
            }
        });
        this.csects.forEach(function (p) {
            if (p.litTab.hasPending()) {
                _this.csects.directives["LTORG"]("AUTO-LTORG", new SicSplit("\tAUTO-LTORG"));
            }
        });
        this.csects.forEach(function (p) {
